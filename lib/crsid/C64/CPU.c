...
unsigned char cRSID_emulateCPU (void) { //the CPU emulation for SID/PRG playback (ToDo: CIA/VIC-IRQ/NMI/RESET vectors, BCD-mode)
 ...
 /* ESP32 port: no longer static — the instance sits in PSRAM behind a
  * runtime pointer, so the address is not a compile-time constant. */
 cRSID_C64instance* const C64 = &cRSID_C64;
 cRSID_CPUinstance* const CPU = &C64->CPU;
