...
/* ESP32: the emulated C64 is too big for a static global, so it is
 * allocated on first cRSID_init(), from PSRAM when the board has it and from
 * internal RAM otherwise. libcRSID.h maps the
 * original `cRSID_C64` global onto this pointer with a macro. */
#include "esp_heap_caps.h"
cRSID_C64instance *cRSID_C64ptr;

/* ESP32 embedding helper: cRSID_generateSample is static inline in this
 * translation unit only — export a buffer-filler for external callers. */
void cRSID_generateBuffer (cRSID_C64instance* C64, signed short* buf, int frames) {
 int i;
 for (i = 0; i < frames; i++) buf[i] = cRSID_generateSample(C64);
}

/* ESP32 port: release the emulated C64 again. The instance is ~270 KB (RAM, both IO
 * banks and the ROM banks), which on a board without PSRAM is most of the free heap —
 * so a firmware that only plays tunes occasionally can hold it just while it plays. */
void cRSID_free (void) {
  if (cRSID_C64ptr != NULL) { heap_caps_free(cRSID_C64ptr); cRSID_C64ptr = NULL; }
}

cRSID_C64instance* cRSID_init (unsigned short samplerate, unsigned short buflen) {
 static cRSID_C64instance* C64;
 if (cRSID_C64ptr == NULL) {
  /* The emulated C64 is ~130 KB (64 KB RAM + 64 KB ROM banks). Prefer PSRAM where
   * a board has it; the Atech 14-port board has none, and its ~360 KB of free
   * internal heap takes it comfortably. */
  cRSID_C64ptr = heap_caps_calloc(1, sizeof(cRSID_C64instance),
                                  MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (cRSID_C64ptr == NULL) {
    cRSID_C64ptr = heap_caps_calloc(1, sizeof(cRSID_C64instance), MALLOC_CAP_8BIT);
  }
  if (cRSID_C64ptr == NULL) return NULL;
 }
 C64 = &cRSID_C64;
