...
/* ESP32 port: the Atech firmware is C++ (Arduino), so give every declaration
 * below C linkage. */
#ifdef __cplusplus
extern "C" {
#endif
...
/* ESP32 port: the C64 instance lives in PSRAM behind a pointer; the macro
 * keeps every `cRSID_C64.member` / `&cRSID_C64` reference source-compatible. */
extern cRSID_C64instance *cRSID_C64ptr;
#define cRSID_C64 (*cRSID_C64ptr)
...
/* ESP32 port: fill a buffer of mono samples. cRSID_generateSample is static inline
 * inside libcRSID.c, so this is the entry point external callers use. */
void cRSID_generateBuffer (cRSID_C64instance* C64, signed short* buf, int frames);

/* ESP32 port: free the instance allocated by cRSID_init (see libcRSID.c). */
